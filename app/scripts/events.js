...
define([
  ...
  "mCustomScrollbar",
  "mousewheel"
], function($, QMCONFIG, Helpers, QMHtml, Location, minEmoji, Ps) {
  var User,
    Dialog,
    Cursor,
    UserView,
  ...
  Events.prototype = {
    init: function() {
      ...
      $(window).focus(function() {
        ...
        var dialogItem = $(
            ".l-list-wrap section:not(#searchList) .is-selected"
          ),
      ...
      $(".popups").on("click", function(event) {
        ...
        if (
          objDom.is(".popups") &&
          !objDom.find(".popup.is-overlay").is(".is-open")
        ) {
          closePopup();
        }
      });
      ...
      $workspace.on("click", ".j-send_location", function() {
        if (localStorage["QM.latitude"] && localStorage["QM.longitude"]) {
          Location.toggleGeoCoordinatesToLocalStorage(false, function(
            res,
            err
          ) {
      ...
      $workspace.on(
        "click",
        ".groupTitle .leaveChat, .groupTitle .avatar",
        function(event) {
          event.stopPropagation();
        }
      );
      ...
      $(document.body).on("click", function() {
        ...
        if ($chat.find(".groupTitle .name_chat").is(".is-focus")) {
          ...
          if (editedChatName && !editedChatName.name) {
            ...
            $chat
              .find(".name_chat")
              .text(editedChatName.name)
              .attr("title", editedChatName.name);
            Dialog.changeName($chat.data("dialog"), editedChatName.name);
          } else {
            $chat.find(".name_chat").text(
              $chat
                .find(".name_chat")
                .text()
                .trim()
      ...
      $(".list, .l-workspace-wrap").on("click", ".j-deleteChat", function() {
        ...
        var $self = $(this),
          parent = $self.parents(".presence-listener")[0]
      ...
      $(".popup-control-button, .btn_popup_private").on("click", function(
        event
      ) {
        event.preventDefault();
      ...
      $(".list").on("click", ".j-requestConfirm", function() {
        var jid = $(this)
          .parents(".j-incomingContactRequest")
          .data("jid");
      ...
      $("#popupContacts .btn_popup_private").on("click", function() {
        var id = $("#popupContacts .is-chosen").data("id"),
          dialogItem = $('.j-dialogItem[data-id="' + id + '"]').find(
            ".contact"
          );
      ...
      $(document).on("keypress", function(event) {
        if (
          event.keyCode === 13 &&
          $(".j-popover_record").hasClass("is-active")
        ) {
  ...
  function clickBehaviour(e) {
    var objDom = $(e.target),
      ...
      googleImage =
        (objDom.context.src &&
          objDom.context.src.indexOf(
            "/maps.gstatic.com/mapfiles/api-3/images/mapcnt6.png"
          )) ||
        null;
  ...
  function removePopover() {
    ...
    if (
      $(".j-start_record").hasClass("is-active") ||
