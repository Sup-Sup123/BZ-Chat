 ...
 * Helper Module
 */
define(["jquery", "underscore", "config", "QBNotification"], function(
  $,
  _,
  QMCONFIG,
  QBNotification
    ...
    getOptions: function(message, params) {
      var myUser = params.user,
        ...
        type =
          ...
          (message.callState && (parseInt(message.callState) + 7).toString()) ||
          "message",
        selectDialog = $(
          '.dialog-item[data-dialog="' + message.dialog_id + '"] .contact'
        ),
        occupants_ids,
        occupantsNames = "",
        options,
      ...
      switch (type) {
        ...
        case "1":
          occupants_ids = _.without(
            message.current_occupant_ids.split(",").map(Number),
            contact.id
          );
          occupantsNames = Helpers.Messages.getOccupantsNames(
            occupants_ids,
            myUser,
            contacts
          );
        ...
        case "7":
          text =
            "You have been deleted from the contact list by " +
            contact.full_name;
          break;
        ...
        case "8":
          if (message.caller === myUser.contact.id) {
            text =
              ...
              ", duration " +
              Helpers.getDuration(message.callDuration);
          } else {
            text =
              "Call from " +
              contacts[message.caller].full_name +
              ", duration " +
              Helpers.getDuration(message.callDuration);
          }
          break;
        ...
        case "9":
          if (message.caller === myUser.contact.id) {
            text =
              "Call to " + contacts[message.callee].full_name + ", no answer";
          } else {
        ...
        case "11":
          text =
            "Incomming " + message.callType + " Call from " + contact.full_name;
          break;
  ...
  Helpers.Messages = {
    getOccupantsNames: function(occupants_ids, myUser, contacts) {
      ...
      for (var i = 0, len = occupants_ids.length, user; i < len; i++) {
        user =
          contacts[occupants_ids[i]] && contacts[occupants_ids[i]].full_name;
        if (user) {
          occupants_names =
            i + 1 === len
              ? occupants_names.concat(user)
              : occupants_names.concat(user).concat(", ");
        } else if (occupants_ids[i] === myContact.id) {
          occupants_names =
            i + 1 === len
              ? occupants_names.concat(myContact.full_name)
              : occupants_names.concat(myContact.full_name).concat(", ");
        }
      }
  ...
  Helpers.Dialogs = {
    moveDialogToTop: function(dialogId) {
      var dialogItem = $(
        '.l-list-wrap section:not(#searchList) .dialog-item[data-dialog="' +
          dialogId +
          '"]'
      );
  ...
  Helpers.getTime = function(time, isDate) {
    ...
    if (messageDate > startOfCurrentDay) {
      ...
      messageDate.getFullYear() === startOfCurrentDay.getFullYear() &&
      !isDate
    ) {
  ...
  Helpers.pauseAllMedia = function(target) {
    document
      .querySelectorAll(".j-audioPlayer, .j-videoPlayer")
