 ...
 *
 */
define(["jquery", "config", "Helpers"], function($, QMCONFIG, Helpers) {
  var curSession, self;
  ...
  VideoChat.prototype.getUserMedia = function(options, callType, callback) {
    ...
    curSession.getUserMedia(params, function(err, stream) {
      if (err) {
        ...
        if (
          !$('.l-chat[data-dialog="' + options.dialogId + '"]').find(
            ".mediacall"
          )[0]
        ) {
          stream.stop({});
  ...
  VideoChat.prototype.sendMessage = function(
    ...
    if (!isErrorMessage) {
      ...
      if (callDuration)
        extension.callDuration = Helpers.getDuration(null, callDuration);
    } else {
      extension = {
        save_to_history: 1,
        date_sent: time,
