 ...
 *
 */
define(["jquery", "underscore", "backbone", "config", "Helpers"], function(
  $,
  _,
  Backbone,
  QMCONFIG,
  ...
  var Person = Backbone.Model.extend({
    ...
    update: function() {
      var currentUser = App.models.User.contact,
        ...
        params = {},
        custom_data =
          (currentUser.custom_data && JSON.parse(currentUser.custom_data)) ||
          {},
        self = this;
      ...
      if (data.status.length >= 0) {
        ...
        params.custom_data = currentUser.custom_data = JSON.stringify(
          custom_data
        );
      }
      if (data.avatar) {
        this.uploadAvatar(data.avatar, function(blob) {
          ...
          $('.profileUserName[data-id="' + currentUser.id + '"]').text(
    ...
    connectFB: function(fbId, callback) {
      ...
      if (self.get("avatar_url") === QMCONFIG.defAvatar.url) {
        custom_data.avatar_url =
          "https://graph.facebook.com/v3.0/" +
          fbId +
          "/picture?width=1200&height=1200";
      }
    ...
    getFBFriends: function() {
      ...
      FB.api("/me/permissions", function(response) {
        ...
        for (var i = 0, len = response.data.length; i < len; i++) {
          if (
            response.data[i].permission === "user_friends" &&
