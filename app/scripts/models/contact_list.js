...
define(["config", "underscore", "Helpers"], function(QMCONFIG, _, Helpers) {
  var contact_ids, isExistingRequest;
      ...
      QBApiCalls.getUser(
        ...
        function(data) {
          ...
          sessionStorage.setItem(
            "QM.search.allPages",
            Math.ceil(data.total_entries / data.per_page)
          );
  ...
  function getContacts() {
    var contacts = {},
      ids = localStorage["QM.contacts"]
        ? localStorage["QM.contacts"].split(",")
        : [];
    ...
    if (ids.length > 0) {
      try {
        for (var i = 0, len = ids.length; i < len; i++) {
          contacts[ids[i]] =
            typeof localStorage["QM.contact-" + ids[i]] !== "undefined"
